...
const List = () => {
  ...
  const fetchLeaveTypes = async () => {
    try {
      ...
      if (Array.isArray(response.data.leaveTypes)) {
  ...
  // Handle adding a leave type (only for admin)
  const handleAddLeaveType = async () => {
    if (!leaveType || !monthlyQuota) {
    ...
    try {
      const response = await axios.post(
        ...
        { leavename: leaveType, monthlyQuota },
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
      ...
      setIsOpen(false);
      fetchLeaveTypes(); // Refresh leave types after addition
    } catch (error) {
  ...
  // Handle filtering by leave type
  useEffect(() => {
    if (selectedType) {
      setFilteredLeaves(
        leaves.filter((leave) => leave.leaveType === selectedType)
      );
    } else {
